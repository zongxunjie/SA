...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		///   the bounded length.
		fn append_or_replace_price(price: u64, price_permill: Permill) {
			Prices::<T>::mutate(|prices| {
				if prices.len() == NUM_VEC_LEN {
					let _ = prices.pop_front();
				}
				prices.push_back((price, price_permill));
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			//let s_info = StorageValueRef::persistent(b"offchain-demo::polkadot-price-data");
			...
			//
			// if let Ok(Some(polkadot_price_data)) = s_info.get::<PolkadotPriceData>() {
			// 	// gh-info has already been fetched. Return early.
			// 	log::info!("cached polkadot-price-data: {:?}", polkadot_price_data);
			// 	return Ok(());
			// }
			...
			if let Ok(_guard) = lock.try_lock() {
				match Self::fetch_n_parse_polkadot() {
					Ok(polkadot_price_data) => {
						// 不签名交易代码
						// s_info.set(&polkadot_price_data);
